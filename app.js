...
const upperTotal   = (sheet) => ['ones','twos','threes','fours','fives','sixes'].reduce((a,k) => a+(sheet[k]??0), 0);
const upperBonus   = (sheet) => upperTotal(sheet) >= 63 ? 35 : 0;
const lowerTotal   = (sheet) => ['three','four','fh','sm','lg','yahtzee','chance'].reduce((a,k) => a+(sheet[k]??0), 0);
...
const aiChooseHolds = (dice, sheet, rollsLeft, difficulty) => {
  if (difficulty === 'easy' && Math.random() < 0.5) return dice.map(() => Math.random() < 0.5);
...
const Stats = {
  ...
  recordGame(players, scores, winnerIdx, you) {
    ...
    if (you !== null) { s.totalScore += scores[you]; if (scores[you] > s.best) s.best = scores[you]; }
    Store.set('stats', s);
  },
...
const showScreen = (id) => {
  ...
  // Scroll to top when changing screens (important on mobile)
  window.scrollTo(0, 0);
};
...
$$('[data-counter="players"] .counter__btn').forEach(b => b.addEventListener('click', () => {
  setPlayerCount(+$('#playerCount').textContent + (+b.dataset.step));
}));
...
const myPlayerId = (() => {
  ...
  if (!id) { id = 'p_'+Math.random().toString(36).slice(2,10)+Date.now().toString(36); localStorage.setItem('yz_pid', id); }
  return id;
})();
...
const initHost = () => {
  ...
  peer.on('open', () => { $('#onlineStatus').textContent = 'Waiting for players…'; renderLobby(); showHostStartButton(); });
  peer.on('error', (err) => {
  ...
  peer.on('connection', (conn) => {
    conn.on('open', () => {
      ...
      if (G.online.started) {
        ...
        if (!slot) { conn.send({ t:'error', msg:'Game already started' }); setTimeout(() => conn.close(), 100); return; }
        slot.conn = conn;
        ...
        renderAll(); wireClientConn(conn); sendStartedStateTo(conn, idx);
        return;
      }

      if (G.online.slots.length >= 8) { conn.send({ t:'error', msg:'Room is full' }); setTimeout(() => conn.close(), 100); return; }
      let slot = G.online.slots.find(s => s.playerId === incomingId);
      if (slot) { slot.conn = conn; slot.name = incomingName; }
      else {
        const idx = G.online.slots.length;
        slot = { playerId: incomingId, name: incomingName, color: AVATAR_COLORS[idx%AVATAR_COLORS.length], conn, isHost: false };
        G.online.slots.push(slot);
      }
      Toast.show(`${slot.name} joined`, 'success');
      conn.send({ t:'welcome', code, yourPlayerId: incomingId });
      renderLobby(); wireClientConn(conn);
    });
    conn.on('error', () => {});
  });
};
...
const renderLobby = () => {
  ...
  G.online.slots.forEach((s) => {
    ...
    const badges = [...(s.isHost ? ['host'] : []), ...(isMe ? ['you'] : [])];
...
const showHostStartButton = () => {
  ...
  btn.id = 'btnHostStart'; btn.className = 'btn primary big'; btn.textContent = 'Start Game';
...
const startOnlineGame = () => {
  ...
  const players = G.online.slots.map((s, i) => ({
    id: 'p'+i, playerId: s.playerId, name: s.name, color: s.color, isAI: false,
    connected: !!s.conn || s.isHost,
  }));
  startGame({ mode: 'online', players });
  const initialStateForClients = buildStatePayload();
  G.online.slots.forEach(s => {
    if (s.conn && !s.isHost) try { s.conn.send({ t:'start', state: initialStateForClients }); } catch(e) { console.error(e); }
  });
};

const buildStatePayload = () => ({
  players: G.players.map(p => ({...p})),
  active: G.active, round: G.round, rollsLeft: G.rollsLeft,
  dice: [...G.dice], held: [...G.held],
  hasRolled: G.hasRolled, selected: G.selected,
...
$('#hostNameInput').addEventListener('input', e => {
  ...
  setStoredName(name);
  if (G.online?.isHost && G.online.slots[0]) { G.online.slots[0].name = name; renderLobby(); }
});
...
$('#btnCopyCode').addEventListener('click', async () => {
  try { await navigator.clipboard.writeText($('#roomCodeDisplay').textContent); Toast.show('Code copied', 'success'); }
  catch { Toast.show('Copy failed — long-press to copy', 'danger'); }
});
...
const doJoin = () => {
  ...
  const peer = new Peer();
  G.online = { peer, code, isHost: false, conn: null, you: null, myName: name, myPlayerId };
  peer.on('open', () => {
    const conn = peer.connect(peerIdFor(code), { metadata:{ name, playerId: myPlayerId }, reliable: true });
    G.online.conn = conn;
    let opened = false;
    conn.on('open', () => { opened = true; $('#joinStatus').textContent = 'Connected — waiting for host to start…'; $('#joinStatus').className = 'status success'; });
    conn.on('data', (msg) => onClientMessage(msg));
    conn.on('close', () => { if (!opened) { $('#joinStatus').textContent = 'Could not connect'; $('#joinStatus').className = 'status error'; return; } handleClientDisconnect(); });
    conn.on('error', () => {});
    setTimeout(() => { if (!opened) { $('#joinStatus').textContent = 'Connection timed out'; $('#joinStatus').className = 'status error'; try { conn.close(); peer.destroy(); } catch {} } }, 12000);
  });
  peer.on('error', (err) => {
    const map = { 'peer-unavailable':'Room not found — check the code', 'network':'Network error — check your connection', 'server-error':'Broker server error — try again' };
...
const handleClientDisconnect = () => {
  ...
  if ($('#screenGame').classList.contains('hidden')) { showScreen('screenStart'); return; }
...
const attemptReconnect = () => {
  ...
  G.online.peer = peer;
  peer.on('open', () => {
    const conn = peer.connect(peerIdFor(code), { metadata:{ name: G.online.myName, playerId: myPlayerId }, reliable: true });
    G.online.conn = conn;
...
const onHostMessage = (conn, msg) => {
  if (msg.t === 'requestState') { sendStartedStateTo(conn); return; }
  ...
  if (msg.playerId !== slot.playerId) return;
...
const onClientMessage = (msg) => {
  if (msg.t === 'welcome') {
    ...
    Toast.show(msg.msg || 'Error', 'danger'); showScreen('screenStart');
  } else if (msg.t === 'start') {
    if (msg.state) { applyStateFromHost(msg.state); }
    else {
      showScreen('screenGame');
      clearTimeout(stateRecvTimer);
      stateRecvTimer = setTimeout(() => { if (G.online?.conn?.open) G.online.conn.send({ t:'requestState', playerId: myPlayerId }); }, 800);
    }
  } else if (msg.t === 'state') {
    clearTimeout(stateRecvTimer); applyStateFromHost(msg.state);
  } else if (msg.t === 'toast') {
    Toast.show(msg.text, msg.kind);
  } else if (msg.t === 'gameover') {
    applyStateFromHost(msg.state); endGame();
  }
};

const applyStateFromHost = (state) => {
  if (!state || !Array.isArray(state.players)) { console.error('[online] invalid state:', state); return; }
  const online = G.online;
  G.mode = 'online'; G.players = state.players; G.active = state.active; G.round = state.round;
  G.rollsLeft = state.rollsLeft; G.dice = state.dice; G.held = state.held;
  G.hasRolled = state.hasRolled; G.selected = state.selected; G.sheets = state.sheets; G.online = online;
...
const onConnClose = (conn) => {
  ...
  if (G.online.started) {
    ...
    if (G.players[idx]) {
      ...
      if (G.active === idx) setTimeout(() => { if (G.active === idx && !G.players[idx].connected) autoSkipTurn(); }, 8000);
    ...
    renderLobby(); Toast.show(`${slot.name} left`, 'danger');
  }
};
...
const autoSkipTurn = () => {
  ...
  G.selected = open.id; doConfirm(); broadcastState();
};

const broadcastState = () => {
  if (G.mode !== 'online' || !G.online?.isHost) return;
  const state = buildStatePayload();
  G.online.slots.forEach(s => { if (s.conn && !s.isHost) try { s.conn.send({ t:'state', state }); } catch(e) { console.error(e); } });
};

const sendStartedStateTo = (conn) => {
  try { conn.send({ t:'state', state: buildStatePayload() }); } catch(e) { console.error(e); }
};
...
function startGame({ mode, players }) {
  const online = G.online;
  G = initialState();
  G.mode = mode; G.players = players; G.sheets = players.map(() => ({})); G.online = online;

  if (mode === 'online') {
    const youIdx = players.findIndex(p => p.playerId === myPlayerId);
    if (youIdx >= 0) { players[youIdx].isYou = true; if (G.online) G.online.you = youIdx; }
  } else {
...
const renderPlayersRail = () => {
  ...
  G.players.forEach((p, i) => {
    ...
    chip.innerHTML = `
      <div class="player-chip__avatar" style="background:${p.color}">${p.isAI ? '🤖' : (p.name[0]||'?').toUpperCase()}</div>
  ...
  const activeChip = rail.querySelector('.player-chip.active');
...
const renderDice = () => {
  ...
  G.dice.forEach((face, i) => {
    ...
    // Touch: prevent double-fire from click on touch devices
    let touchMoved = false;
    die.addEventListener('touchstart', () => { touchMoved = false; }, { passive: true });
    die.addEventListener('touchmove', () => { touchMoved = true; }, { passive: true });
...
const renderRollsLeft = () => {
  ...
  el.appendChild(Object.assign(document.createElement('span'), {
    textContent: ` ${G.rollsLeft} ${G.rollsLeft===1?'roll':'rolls'} left`,
  }));
};

const renderActions = () => {
  const isMyTurn = isMyTurnToControl(G.active) && !G.players[G.active].isAI;
  const canRoll = isMyTurn && G.rollsLeft > 0 && !(!G.hasRolled ? false : false);
  const canConfirm = isMyTurn && G.hasRolled && G.selected !== null;

  $('#btnRoll').classList.toggle('hidden', !canRoll);
  $('#btnConfirm').classList.toggle('hidden', !canConfirm);
  $('#btnRoll').disabled = !canRoll;

  // Update roll button text to include rolls left on mobile
  if (canRoll) {
    const dots = '●'.repeat(G.rollsLeft) + '○'.repeat(3 - G.rollsLeft);
    $('#btnRoll').innerHTML = `🎲 Roll <small style="font-size:.75em;opacity:.7">${dots}</small>`;
  }
};
...
const renderScoresheet = () => {
  ...
  G.players.forEach((p, i) => {
    html += `<th class="${i===G.active?'col-active':''}" style="color:${p.color}" title="${escapeHtml(p.name)}">${escapeHtml(p.name).slice(0,8)}</th>`;
  });
  html += '</tr></thead><tbody>';
  ...
  G.players.forEach((_, i) => {
    ...
    const display = earned
      ? `${t} <span class="bonus-add">+35</span>`
      : `${t}<span class="bonus-target">/63</span>`;
  ...
  $$('.preview', sheet).forEach(td => {
    td.addEventListener('click', () => selectCategory(td.dataset.cat));
    // Prevent ghost-click on touch
    let tm = false;
    td.addEventListener('touchstart', () => { tm = false; }, { passive: true });
    td.addEventListener('touchmove', () => { tm = true; }, { passive: true });
    td.addEventListener('touchend', (e) => { if (tm) return; e.preventDefault(); selectCategory(td.dataset.cat); });
  });
};
...
const scoreCellHtml = (cat, playerIdx) => {
  ...
  let classes = isActive ? ' col-active' : '';

  if (scored !== undefined) return `<td class="${classes} scored">${scored}</td>`;
...
const sendAction = (action) => {
  ...
  else if (G.online.conn?.open) G.online.conn.send({ t:'action', playerId: myPlayerId, action });
};
...
const doRoll = () => {
  ...
  G.rollsLeft--; G.hasRolled = true; G.selected = null;
  $$('.die').forEach((el, i) => {
    if (!G.held[i]) {
...
const doSelect = (catId) => {
  ...
  if (G.sheets[G.active][catId] !== undefined) return;
  G.selected = catId; Sound.tick(); renderAll();
};
...
const doConfirm = () => {
  ...
  prevSnapshot = JSON.stringify({ active: G.active, round: G.round, sheets: G.sheets.map(s=>({...s})) });
  ...
  sheet[catId] = points; Sound.confirm();
...
const advanceTurn = () => {
  ...
  if (G.players.every((_,i) => sheetComplete(G.sheets[i]))) return endGame();
  renderAll();
...
$('#btnUndo').addEventListener('click', () => {
  if (!prevSnapshot || G.players[G.active]?.isAI || G.mode==='online') return;
  const snap = JSON.parse(prevSnapshot);
  G.active=snap.active; G.round=snap.round; G.sheets=snap.sheets;
  G.dice=[0,0,0,0,0]; G.held=[false,false,false,false,false];
  G.rollsLeft=3; G.hasRolled=false; G.selected=null; prevSnapshot=null;
  Toast.show('Turn undone', 'success'); renderAll();
});
$('#btnMenu').addEventListener('click', () => {
  if (!confirm('Quit current game and return to menu?')) return;
  teardownOnline(); showScreen('screenStart');
});

/* ---------- KEYBOARD ---------- */
addEventListener('keydown', (e) => {
  if ($('#screenGame').classList.contains('hidden')) return;
  if (G.players[G.active]?.isAI || !isMyTurnToControl(G.active)) return;
...
const aiThink = () => {
  ...
  if (G.rollsLeft > 0) {
    ...
    holds.forEach((h, i) => { if (h !== G.held[i]) sendAction({ type:'toggleHold', idx: i }); });
    setTimeout(() => {
      if (G.rollsLeft > 0) { sendAction({ type:'roll' }); setTimeout(() => aiThink(), 900); }
      else aiChoose();
    }, 700);
  } else {
    aiChoose();
...
const endGame = () => {
  ...
  History.add({ when: Date.now(), players: G.players.map((p,i) => ({ name:p.name, score:scores[i], isAI:p.isAI })), winner: G.players[winnerIdx].name });
  ...
  if (you >= 0 && CATEGORIES.every(c => G.sheets[you][c.id] > 0)) Achievements.unlock('flawless');

  Sound.win(); Confetti.burst();

  const list = G.players.map((p,i) => ({ name:p.name, score:scores[i], color:p.color })).sort((a,b) => b.score-a.score);
...
window.__playAgain = () => {
  ...
  if (G.mode==='online' && G.online?.isHost) {
    ...
    startGame({ mode:'online', players });
    const state = buildStatePayload();
    G.online.slots.forEach(s => { if (s.conn && !s.isHost) try { s.conn.send({ t:'start', state }); } catch(e){console.error(e);} });
  } else {
...
$('#btnRules').addEventListener('click', () => openModal(`
  <h2>How to Play</h2>
  <p>On each turn, roll up to <b>3 times</b>, holding the dice you want to keep. Then choose a category to score in.</p>
  <h3>Upper Section</h3>
  <p>Sum of dice matching the number. Score <b>63+</b> to earn a <b>+35 bonus</b>.</p>
  <h3>Lower Section</h3>
  <ul>
    <li><b>3/4 of a kind</b> — sum of all dice</li>
    <li><b>Full House</b> — 25 pts (3+2)</li>
    <li><b>Small Straight</b> — 30 pts (4 in a row)</li>
    <li><b>Large Straight</b> — 40 pts (5 in a row)</li>
    <li><b>Yahtzee</b> — 50 pts. Extra Yahtzees = +100 each.</li>
...
if (/#room=[A-Z0-9]{6}/i.test(location.hash)) setTimeout(() => openSetup('online-join'), 100);
